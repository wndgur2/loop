# ADR 0004 — Loopie 응답 스트리밍 (SSE)

작성일: 2026-06-08 · 상태: 채택(Accepted)
관련: [adr-0002](adr-0002-backend-structure.md) · [loopie-spec.md](../documents/loopie-spec.md) · [CLAUDE.md](../CLAUDE.md) §6

## 맥락

Loopie 답변은 한 번에 JSON(`{reply, proposal}`)으로 돌아와, 사용자는 모델이 끝까지 생성할 때까지 빈 화면(스피너)만 봤다. 대화형 회고에선 첫 글자가 빨리 보이는 체감 지연이 품질에 직결된다. 모든 AI 호출은 Edge Function 경유라는 전제(§6)는 유지한다.
...
2. **env는 원인 아님(레드헤링).** `mobile/.env`가 운영을 가리켜 구버전(JSON) 함수가 응답하던 건 별개로 [loopie.ts](../mobile/src/lib/loopie.ts) JSON 폴백으로 방어. `GEMINI_MODEL` 값의 인라인 주석(`# stable…`)도 dotenv가 떼고 읽어 무해.
