# ADR 0003 — MVP 클라이언트 (4탭 + 피드백 작성·조회 + Loopie 채팅)
...
ADR-0002의 백엔드(스키마·Edge Function) 위에 Expo 모바일 클라이언트 MVP를 올렸다. 범위: **4개 탭(피드백·회고·대시보드·설정) + 피드백 작성·조회 + Loopie 대화(작성/회고)**. 디자인 정본은 `demo/`(Claude Design handoff)의 "정돈된 따뜻함" 방향.
...
- **Loopie 채팅**: Edge Function이 스택에서 서빙됨(인증 게이트 401 확인). 실제 AI 응답은 `ANTHROPIC_API_KEY`(로컬 `supabase/functions/.env`) 설정 시 완결.
