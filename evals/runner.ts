...
// --- 시나리오 / 출력 / 채점 타입 (ai-coaching-spec.md, data-model.md와 일치) ---
...
/** ai-coaching-spec.md 4절 구조화 출력 계약 (write 모드 산출물 = Canonical Template) */
...
/** 코칭 Edge Function을 호출해 대화를 진행한다. write는 구조화 출력, retrospective는 상태 갱신 의도를 반환. */
async function runCoaching(_scenario: Scenario): Promise<{
  transcript: { role: "user" | "assistant"; content: string }[];
  output: CoachingOutput | null; // write 모드에서만 채워짐
}> {
  // TODO: supabase/functions/coaching 호출 (mode 분기). 프롬프트 캐싱 적용.
