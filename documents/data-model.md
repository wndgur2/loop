...
profiles (1) ──< chat_sessions ──< chat_messages
...
| `id` | uuid (PK) | |
| `user_id` | uuid (FK→profiles) | |
| `session_id` | uuid (FK→chat_sessions) | nullable (직접 생성 시 null) |
...
- 자식 테이블(`sub_goals`, `takeaways`, `chat_messages`)은 부모를 통해 소유권 확인(부모의 `user_id = auth.uid()`를 EXISTS로).
