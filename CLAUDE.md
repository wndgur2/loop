...
- ☐ **Supabase 프로젝트 연결** + 초기 마이그레이션(profiles/goals/sub_goals/feedbacks/takeaways/chat_*) — `supabase-migration` 스킬(RLS 강제), 정본은 [데이터 모델](documents/data-model.md)
- ☐ 마이그레이션 후 `supabase gen types` → `mobile/src/types/database.ts` 재생성(현재 빈 placeholder)
- ☐ Loopi Edge Function 골격 + 프롬프트 디렉토리([supabase/functions/README.md](supabase/functions/README.md))
