...
> 관련 문서: [PRD](documents/PRD.md) · [기능 명세](documents/feature-spec.md) · [브랜딩](documents/branding.md) · [데이터 모델](documents/data-model.md) · [Loopie 스펙](documents/loopie-spec.md) · [평가 기준](documents/eval-rubric.md)
...
**Loop** — 스스로 남긴 피드백을 Loopie가 구조화해 "되돌아보기"를 "목표 달성"으로 연결하는 모바일 앱.
...
루트에는 **문서·공유 백엔드·공유 패키지**를 두고, **플랫폼별 앱은 각자 디렉토리**로 분리한다. 현재 앱은 `mobile/`(Expo). 이후 다른 플랫폼(예: web)은 같은 `supabase/` 백엔드와 `@loop/ui` 디자인 시스템을 공유하는 형제 디렉토리로 추가한다. 저장소는 **npm workspaces 모노레포**다(루트 `package.json`의 `workspaces: ["mobile", "packages/*"]`) — [adr-0005](adrs/adr-0005-design-system-package.md).

```
loop/
├── package.json         # npm workspaces 루트 (mobile + packages/*) · RN 스택 버전 overrides
├── CLAUDE.md            # 이 문서
├── documents/           # 제품·브랜드 문서 (PRD, 브랜딩, 데이터 모델, Loopie 스펙, 평가 기준)
├── adrs/                # 설계 결정 기록(ADR) — 0001 스캐폴딩 · 0002 백엔드 · 0003 MVP 클라 · 0004 스트리밍 · 0005 @loop/ui 분리
├── demo/                # 디자인 정본(hi-fi 목업, React+CSS 웹) — 색/타입/컴포넌트 톤의 출처
├── .claude/skills/      # 프로젝트 전용 스킬 (supabase-migration, eval-loopie, react-native-expert)
├── evals/               # Loopie 품질 평가 루프 (시나리오 + 러너) — 플랫폼 공유
├── packages/
│   └── design-system/   # @loop/ui — 디자인 시스템(토큰·아이콘·UI 프리미티브), 모든 플랫폼 공유 (src/tokens · src/components · index.ts 배럴)
├── supabase/            # 공유 백엔드 (모든 플랫폼이 사용)
│   ├── migrations/      #   DB 스키마 (SQL) — RLS 필수 (init + grant_api_roles)
│   ├── functions/       #   Edge Functions — Loopie 대화 엔진
...
└── mobile/              # Expo 앱 (이 플랫폼)
    ...
    ├── metro.config.js  #   모노레포 Metro 설정 (@loop/ui 소스 해석·감시)
    ├── .env.example     #   클라이언트 공개 값 템플릿 (EXPO_PUBLIC_*)
    ├── assets/          #   아이콘·스플래시·폰트(Pretendard)·탭 아이콘 등 정적 자산 (app.json이 참조)
    └── src/
        ├── app/         #   Expo Router 화면 — (tabs)/ · chat/[mode] · feedback/[id]·new · onboarding · sign-in
        ├── components/  #   앱 전용 컴포넌트 — themed-text/view, skeletons(도메인 프리셋). 공용 UI는 @loop/ui
        ├── features/    #   도메인별 묶음 (auth, feedback, goals, dashboard, reflect, chat)
        ├── lib/         #   supabase·query-client·query-keys, loopie(Edge 래퍼), i18n·translations, date
        ├── hooks/       #   공용 훅 (use-theme, use-color-scheme)
        ├── constants/   #   theme(레거시 라이트/다크). 브랜드 토큰은 @loop/ui
        └── types/       #   database.ts(Supabase 생성) · models.ts(camelCase 도메인 + Row 매퍼)
```

> 앱 코드는 `mobile/` 안에서 작업한다(라우터 루트 `mobile/src/app/`, alias `@/*` → `mobile/src/*`). **공용 UI·디자인 토큰은 `@loop/ui`** 에서 import한다(앱 alias가 아니라 워크스페이스 패키지). npm 설치/게이트는 **워크스페이스 루트**에서 돈다(§4).
> 디자인 시스템(`packages/design-system` = `@loop/ui`)은 **앱 도메인을 모른다** — i18n·쿼리·라우터·`@/...` 의존 금지. 새 공용 컴포넌트는 거기에 추가하고 `index.ts` 배럴에 export한다([README](packages/design-system/README.md)).
...
```bash
# --- 설치·게이트: npm workspaces 루트에서 ---
npm install            # 루트에서 1회 (mobile + packages/*). @loop/ui 심링크 + RN 스택 dedupe(overrides)
npm run typecheck      # @loop/ui + mobile 둘 다 (tsc --noEmit)
npm run lint           # @loop/ui + mobile 둘 다
...
**작업을 마치기 전 반드시 루트에서 `npm run typecheck`·`npm run lint`(=`@loop/ui`+`mobile` 양쪽)를 통과시키세요.** `@loop/ui`를 건드렸다면 두 워크스페이스 모두 영향받습니다. Edge Function을 건드렸다면 `deno check`도 통과시키세요. 실패하면 그 사실을 그대로 보고하고, 임의로 무시하거나 타입을 우회하지 마세요.
...
- **Loopie는 DB를 직접 바꾸지 않습니다.** Edge Function은 `proposal`만 반환하고, 클라이언트가 사용자 확인을 받아 RLS mutation으로 반영합니다(조용한 변경 금지).
...
- **작업은 feature 브랜치에서 진행**하고 `main`에 직접 푸시하지 마세요. `main` 반영은 PR로 하고, CI(typecheck·lint·format:check + Edge `deno check`)가 초록인 걸 확인한 뒤 머지합니다(머지 후 브랜치 삭제). `main`은 브랜치 룰로 force-push·삭제가 차단돼 있습니다.
- CI/CD 워크플로는 `.github/workflows/`에 있습니다 — `ci`(PR 게이트) · `cd-backend`(`supabase/**` 변경 시 main에서 자동 배포) · `cd-mobile`(수동: ota/build/build-and-submit).
...
- Loopie 품질이 차별점이므로, **프롬프트는 `supabase/functions` 내에 모아 버전 관리**하고 임의로 흩뜨리지 마세요.
...
- ✅ Loopie Edge Function 골격 + 프롬프트 디렉토리([supabase/functions/README.md](supabase/functions/README.md)) — adr-0002
- ✅ **MVP 클라이언트**: 인증(F1)·온보딩·4탭(피드백/회고/인사이트/설정)·피드백 작성(직접·Loopie)·조회 — [adr-0003](adrs/adr-0003-mvp-client.md), web 프리뷰 실측 검증
- ✅ **LLM 프로바이더 어댑터** — gemini(기본)/anthropic/openai, `LLM_PROVIDER`로 교체 ([functions/README](supabase/functions/README.md))
- ✅ **Loopie 응답 SSE 스트리밍** — Edge Function·클라(`expo/fetch`) 토큰 단위, JSON 폴백 — [adr-0004](adrs/adr-0004-streaming.md)
- ✅ 이메일 확인 딥링크 인증 + Android EAS 빌드 설정(`eas.json`)
- ✅ 경량 i18n(한/영) · `grant_api_roles` 마이그레이션(anon/authenticated 테이블 권한)
- ✅ **Google Play 정책 대응(인앱)** — 개인정보처리방침·이용약관 인앱 표시(`/legal/[doc]`, 정본 `documents/legal/`), 가입 동의 고지, AI 생성 고지 + Loopie 답변 길게 눌러 신고(`ai_content_reports`)
- ✅ **디자인 시스템 패키지 분리(`@loop/ui`)** — npm workspaces, 토큰+컴포넌트를 `packages/design-system`으로 이동, 컴포넌트 확장(레이아웃·폼·피드백/오버레이·데이터표시 24종) — [adr-0005](adrs/adr-0005-design-system-package.md), web 프리뷰 실측 검증
- ☐ **다음 EAS 릴리스 전 워크스페이스 빌드 검증** — 모노레포 전환이 모듈 해석을 바꿈. `eas build --profile preview`로 한 번 확인(미검증 시 릴리스 빌드 깨질 수 있음)
- ☐ Play 제출 잔여 작업 — `supabase db push`로 `ai_content_reports` 마이그레이션을 호스팅 프로젝트에 적용(미적용 시 신고 기능이 런타임에 실패), `documents/legal/privacy-policy.md` 공개 URL 호스팅 후 Play Console 개인정보처리방침 필드 등록, Data safety 폼 작성(수집: 이메일·이름·사용자 콘텐츠 / 공유: LLM 제공자 / 삭제: 인앱+이메일), 법무 검토
