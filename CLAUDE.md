...
├── evals/               # AI 코칭 품질 평가 루프 (시나리오 + 러너) — 플랫폼 공유
...
- ☐ **Supabase 프로젝트 연결** + 초기 마이그레이션(profiles/goals/sub_goals/feedbacks/takeaways/coaching_*) — `supabase-migration` 스킬(RLS 강제), 정본은 [데이터 모델](documents/data-model.md)
- ☐ 마이그레이션 후 `supabase gen types` → `mobile/src/types/database.ts` 재생성(현재 빈 placeholder)
- ☐ Claude 코칭 Edge Function 골격 + 프롬프트 디렉토리([supabase/functions/README.md](supabase/functions/README.md))
- ☐ 인증(F1) · 온보딩 · 화면(피드백/회고/대시보드/설정) — [기능 명세](documents/feature-spec.md) 화면 인벤토리
