...
| AI | **Claude API (Anthropic)** — 운영 기본 | **반드시 서버(Edge Function) 경유** — 키를 클라이언트에 두지 않음. 어댑터로 openai/gemini도 교체 가능(`LLM_PROVIDER`) |
...
> AI 프로바이더는 어댑터(`supabase/functions/chat/llm/`)로 추상화돼 호출부는 프로바이더를 모릅니다. 운영 기본은 Anthropic이며, 키를 가진 프로바이더만 설정합니다(§6).
...
- ✅ **LLM 프로바이더 어댑터** — anthropic(기본)/openai/gemini, `LLM_PROVIDER`로 교체 ([functions/README](supabase/functions/README.md))
