...
│   └── types.ts         # 요청/응답 계약 (CoachingRequest/Response, proposal 타입)
└── coaching/            # AI 코칭 (작성·회고 공통 엔진, 모드당 툴 1개)
    ...
    └── prompts/         #   시스템 프롬프트 — import되는 TS 모듈로 버전 관리
        ├── system.ts           # 페르소나 (ai-coaching-spec §2)
...
- 기본 모델 `claude-opus-4-8`(secret `COACHING_MODEL`로 오버라이드). 키는 `ANTHROPIC_API_KEY` secret.
